    ...
    expect(
      ...
      OkResult {
        "value": Object {
          ...
          "pagingCount": 25,
          "text": "SELECT e.id, e.uuid, e.type, e.name, ev.version, ev.data
        FROM entities e, entity_versions ev WHERE e.latest_draft_entity_versions_id = ev.id AND jsonb_to_tsvector(ev.data, '[\\"string\\", \\"numeric\\"]') @@ websearch_to_tsquery($1) ORDER BY e.id LIMIT $2",
          "values": Array [
            "foo bar",
            26,
          ],
...
describe('totalAdminEntitiesQuery()', () => {
  ...
  test('query text', () => {
    expect(
      ...
      OkResult {
        "value": Object {
          "text": "SELECT COUNT(e.id)::integer AS count FROM entities e, entity_versions ev WHERE e.latest_draft_entity_versions_id = ev.id AND jsonb_to_tsvector(ev.data, '[\\"string\\", \\"numeric\\"]') @@ websearch_to_tsquery($1)",
          "values": Array [
            "foo bar",
          ],
        },
