...
describe('adminEntitySearchEntities', () => {
  test('Minimal, no results', async () => {
    ...
    expect(getRunAndQueryCalls(innerAdapter)).toMatchInlineSnapshot(`
      [
        [
          "WITH entities_cte AS (SELECT e.id, e.uuid, e.type, e.name, e.auth_key, e.created_at, e.updated_at, e.updated_seq, e.status, e.invalid, e.latest_entity_versions_id FROM entities e WHERE e.uuid IS NOT NULL AND e.resolved_auth_key = ?1 AND e.status != 'archived' ORDER BY e.id LIMIT ?2)
  ...
  test('Paging after, one result', async () => {
    ...
    expect(getRunAndQueryCalls(innerAdapter)).toMatchInlineSnapshot(`
      [
        [
          "WITH entities_cte AS (SELECT e.id, e.uuid, e.type, e.name, e.auth_key, e.created_at, e.updated_at, e.updated_seq, e.status, e.invalid, e.latest_entity_versions_id FROM entities e WHERE e.uuid IS NOT NULL AND e.resolved_auth_key = ?1 AND e.status != 'archived' AND e.id > ?2 ORDER BY e.id LIMIT ?3)
      SELECT e.*, ev.version, ev.schema_version, ev.encode_version, ev.fields FROM entities_cte e JOIN entity_versions ev ON e.latest_entity_versions_id = ev.id",
          "",
          1,
          11,
