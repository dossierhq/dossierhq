...
describe('generateGetChangelogEventsQuery', () => {
  test('default', () => {
    expect(getChangelogEventsQuery().valueOrThrow()).toMatchInlineSnapshot(`
      {
        "text": "SELECT e.id, e.uuid, e.type, e.created_at, s.uuid AS created_by, sv.version FROM events e JOIN subjects s ON e.created_by = s.id LEFT JOIN schema_versions sv ON e.schema_versions_id = sv.id ORDER BY e.id LIMIT ?1",
        "values": [
          26,
        ],
      }
  ...
  test('reverse', () => {
    expect(getChangelogEventsQuery({ reverse: true }).valueOrThrow()).toMatchInlineSnapshot(`
      {
        "text": "SELECT e.id, e.uuid, e.type, e.created_at, s.uuid AS created_by, sv.version FROM events e JOIN subjects s ON e.created_by = s.id LEFT JOIN schema_versions sv ON e.schema_versions_id = sv.id ORDER BY e.id DESC LIMIT ?1",
        "values": [
          26,
        ],
      }
  ...
  test('createdBy', () => {
    expect(getChangelogEventsQuery({ createdBy: '1-2-3' }).valueOrThrow()).toMatchInlineSnapshot(`
      {
        "text": "SELECT e.id, e.uuid, e.type, e.created_at, s.uuid AS created_by, sv.version FROM events e JOIN subjects s ON e.created_by = s.id LEFT JOIN schema_versions sv ON e.schema_versions_id = sv.id WHERE e.created_by = (SELECT id FROM subjects WHERE uuid = ?1) ORDER BY e.id LIMIT ?2",
        "values": [
          "1-2-3",
          26,
        ],
  ...
  test('types', () => {
    expect(getChangelogEventsQuery({ types: [EventType.updateSchema] }).valueOrThrow())
      .toMatchInlineSnapshot(`
        {
          "text": "SELECT e.id, e.uuid, e.type, e.created_at, s.uuid AS created_by, sv.version FROM events e JOIN subjects s ON e.created_by = s.id LEFT JOIN schema_versions sv ON e.schema_versions_id = sv.id WHERE e.type IN (?1) ORDER BY e.id LIMIT ?2",
          "values": [
            "updateSchema",
            26,
          ],
  ...
  test('types and createdBy', () => {
    expect(
      ...
      {
        "text": "SELECT e.id, e.uuid, e.type, e.created_at, s.uuid AS created_by, sv.version FROM events e JOIN subjects s ON e.created_by = s.id LEFT JOIN schema_versions sv ON e.schema_versions_id = sv.id WHERE e.created_by = (SELECT id FROM subjects WHERE uuid = ?1) AND e.type IN (?2) ORDER BY e.id LIMIT ?3",
        "values": [
          "1-2-3",
          "updateSchema",
          26,
