...
registerTestSuite(
  ...
  createSyncTestSuite({
    before: async () => {
      const sourceServer = (await initializeEmptyIntegrationTestServer(':memory:')).valueOrThrow();
      const targetServer = (await initializeEmptyIntegrationTestServer(':memory:')).valueOrThrow();
      return [
