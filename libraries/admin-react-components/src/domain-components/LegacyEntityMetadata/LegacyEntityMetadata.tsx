...
export function LegacyEntityMetadata({
  entityId,
  className,
  initialSelectedHistory,
}: LegacyEntityMetadataProps): JSX.Element {
    ...
    >
      ...
      <ColumnItem className="dd-mx-2">
        ...
        <Row>
          ...
          {status ? <LegacyPublishStateTag publishState={status} /> : null}
        </Row>
        <LegacyPublishingButton
          {...{
            entityId,
...
function EntityHistoryList({ draftState }: { draftState: LegacyEntityEditorDraftState }) {
  const { useEntityHistory } = useContext(LegacyDataDataContext);
...
function PublishButton({
  ...
  const { publishEntities } = useContext(LegacyDataDataContext);
...
function PublishingHistory({ draftState }: { draftState: LegacyEntityEditorDraftState }) {
  const { usePublishingHistory } = useContext(LegacyDataDataContext);
