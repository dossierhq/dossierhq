...
> {
  ...
  type EntityRow = Pick<
    EntitiesTable,
    'id' | 'uuid' | 'auth_key' | 'resolved_auth_key' | 'status' | 'latest_draft_entity_versions_id'
  >;
  const entityResult = await queryMany<EntityRow>(
    ...
    buildPostgresSqlQuery(
      ({ sql }) =>
        sql`SELECT e.id, e.uuid, e.auth_key, e.resolved_auth_key, e.status, e.latest_draft_entity_versions_id
            FROM entities e
            WHERE e.uuid = ANY(${uuids})`,
    ),
  );
  ...
  for (const uuid of uuids) {
    ...
    if (!row) {
      ...
      payload.push({
        ...
        entityVersionInternalId: row.latest_draft_entity_versions_id,
    ...
    buildPostgresSqlQuery(
      ({ sql }) =>
        sql`SELECT elr.to_entities_id, e.uuid
          ...
          WHERE elr.to_entities_id = ANY(${payload.map((it) => it.entityInternalId as number)})`,
    ),
  );
