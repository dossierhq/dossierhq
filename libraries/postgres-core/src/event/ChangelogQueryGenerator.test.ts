...
describe('generateGetChangelogEventsQuery', () => {
  ...
  test('types', () => {
    expect(getChangelogEventsQuery({ types: [EventType.updateSchema] }).valueOrThrow())
      .toMatchInlineSnapshot(`
        {
          "text": "SELECT e.id, e.type, e.created_at, s.uuid, sv.version FROM events e JOIN subjects s ON e.created_by = s.id LEFT JOIN schema_versions sv ON e.schema_versions_id = sv.id WHERE e.type = ANY($1) ORDER BY e.id LIMIT $2",
          "values": [
            [
              "updateSchema",
            ],
  ...
  test('types and createdBy', () => {
    expect(
      ...
      {
        "text": "SELECT e.id, e.type, e.created_at, s.uuid, sv.version FROM events e JOIN subjects s ON e.created_by = s.id LEFT JOIN schema_versions sv ON e.schema_versions_id = sv.id WHERE e.created_by = (SELECT id FROM subjects WHERE uuid = $1) AND e.type = ANY($2) ORDER BY e.id LIMIT $3",
        "values": [
          "1-2-3",
          [
            "updateSchema",
