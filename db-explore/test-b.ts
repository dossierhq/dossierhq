...
async function randomReference(query: Core.Query) {
  const result = await CoreB.getRandomEntity(query);
...
async function createOrganization(session: Core.Session, timestamp: string) {
  const result = await BenchPress.runTest(
    async (clock) => {
      ...
      clock.start();
      await CoreB.createEntity(session, 'Organization', name, entity);

      clock.stop();
      return true;
    },
    { name: 'create organization', warmup: 30, iterations: 10 }
  );

  await BenchPress.reportResult(result, {
    percentiles: PERCENTILES,
    folder: path.join(__dirname, 'output'),
    baseName: `b-create-organization-${timestamp}`,
  });
}
...
async function createPlaceOfBusiness(session: Core.Session, timestamp: string) {
  const result = await BenchPress.runTest(
    async (clock) => {
      ...
      await CoreB.createEntity(session, 'PlaceOfBusiness', name, entity);
      clock.stop();
      return true;
    },
    { name: 'create place-of-business', warmup: 30, iterations: 10 }
  );

  await BenchPress.reportResult(result, {
    percentiles: PERCENTILES,
    folder: path.join(__dirname, 'output'),
    baseName: `b-create-place-of-business-${timestamp}`,
  });
}
